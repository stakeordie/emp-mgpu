...
# Added: 2025-06-04T12:45:54-04:00 - Made all_services_ok a global variable to fix verification reporting
...
verify_services() {
    ...
    # Added: 2025-06-03T22:17:27-04:00 - Replaced Langflow verification with Ollama verification
    ...
    # Method 1: Check using process search
    if pgrep -f "ollama serve" >/dev/null; then
        log "Ollama process is running (process check)"
        ollama_running=true
    else
        log "NOTE: Ollama process not found via process check"
    fi
    
    # Method 2: Direct port check
    if netstat -tuln | grep -q ":$ollama_port "; then
        log "Ollama port $ollama_port is listening (port check)"
        ollama_running=true
    else
        log "NOTE: Ollama port $ollama_port not listening"
    fi
    
    # Method 3: API check
    if curl -s -o /dev/null -w "%{http_code}" http://$ollama_host:$ollama_port/api/tags | grep -q "200"; then
        log "Ollama is responding to API requests (API check)"
        ollama_running=true
    else
        log "NOTE: Ollama not responding to API requests"
    fi
    
    # Final decision based on all checks
    if [ "$ollama_running" = "true" ]; then
        log "SUCCESS: Ollama is running (at least one check passed)"
    else
        log "ERROR: Ollama is not running (all checks failed)"
        all_services_ok=false
    ...
    # Added: 2025-06-03T22:17:27-04:00 - Replaced langflow status with ollama status
    log "Ollama: $([ "$ollama_running" = "true" ] && echo "RUNNING" || echo "NOT RUNNING")"
    ...
    # Added: 2025-06-03T22:46:12-04:00 - Enhanced error reporting
    # Return final status with detailed error information
    if [ "$all_services_ok" = true ]; then
        log "All services are running correctly"
        return 0
    else
        ...
        # Print the failed services with more detail
        log "==== DETAILED FAILURE REPORT ===="
        for service in "${failed_services[@]}"; do
            log "FAILED: $service"
            
            # For ComfyUI instances, check logs for specific errors
            if [[ "$service" == *"ComfyUI"* ]] && [[ "$service" == *"instance"* ]]; then
                instance=$(echo "$service" | grep -o '[0-9]\+')
                if [ -n "$instance" ]; then
                    log "  Checking ComfyUI logs for instance $instance:"
                    tail -n 20 "/workspace/logs/comfyui_${instance}.log" 2>/dev/null | grep -i "error\|exception\|failed" | head -n 5 | while read -r line; do
                        log "    ERROR: $line"
                    done
                fi
            fi
            
            # For A1111 instances, check logs for specific errors
            if [[ "$service" == *"Automatic1111"* ]] && [[ "$service" == *"instance"* ]]; then
                instance=$(echo "$service" | grep -o '[0-9]\+')
                if [ -n "$instance" ]; then
                    log "  Checking A1111 logs for instance $instance:"
                    tail -n 20 "/workspace/logs/a1111_${instance}.log" 2>/dev/null | grep -i "error\|exception\|failed" | head -n 5 | while read -r line; do
                        log "    ERROR: $line"
                    done
                fi
            fi
            
            # For Redis workers, check logs for specific errors
            if [[ "$service" == *"Redis Worker"* ]]; then
                instance=$(echo "$service" | grep -o '[0-9]\+')
                if [ -n "$instance" ]; then
                    log "  Checking Redis Worker logs for instance $instance:"
                    tail -n 20 "/workspace/logs/worker_${instance}.log" 2>/dev/null | grep -i "error\|exception\|failed" | head -n 5 | while read -r line; do
                        log "    ERROR: $line"
                    done
                fi
            fi
